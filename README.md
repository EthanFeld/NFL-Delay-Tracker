...
A hard zero is published only when a fresh NWS or HREF thunder forecast covers every five-minute bin in the game exposure and reports zero throughout. During an exposure already underway, fresh local MRMS probabilities, lightning density, and the active-warning feed must also confirm clear conditions; an approaching radar track or active delay blocks the hard zero. Missing or stale coverage cannot prove a storm-free game, so the forecast stays unavailable instead of reporting zero. Any positive thunder probability inside the game window remains eligible for a nonzero estimate.

When a venue is outside NWS coverage or a game is more than seven days away, Open-Meteo's global ECMWF IFS ensemble may provide weather-code member counts through the schedule's 14-day window. Its roughly 25 km grid resolves every 3 hours through day 6 and every 6 hours afterward. It provides conditions only; it cannot estimate thunderstorms, lightning delay odds, or storm motion. Those games show no numeric delay risk.

In-progress delay odds now start at refresh time and stop at an estimated remaining-game horizon from the live quarter and clock; the model no longer re-simulates completed play. It scales the remaining regulation clock against its 210-minute full-game exposure, allows one 10-minute overtime period for a tied NFL game in the final minute, and caps college overtime at 30 model minutes because possession-based periods have no continuous clock. If scoreboard data is absent, elapsed time reduces the horizon during live play; during a weather hold, the full current period is used because the game clock is paused. If the period is also missing during a hold, the estimate uses the full-game horizon. These live odds remain experimental and uncalibrated. The JSON retains the legacy `pregame` field name for live forecasts, but live delay probability and expected delay minutes cover remaining exposure from the snapshot time. Active-delay estimates use fresh MRMS density and future hazard bins when available. During active games, the tracker also links connected MRMS reflectivity objects across recent scans, then estimates bearing, radial motion and constant-speed ETA to the configured policy boundary. A fresh approaching echo can add a small capped hazard pulse only in the 30–180 minute window. Radar echo is not lightning; ETA bounds are sensitivity estimates, and this probability adjustment has not been calibrated. The historical delay corpus contains 29 sourced positive events (25 in the priority range), but lacks archived storm tracks, so backtesting validates only the broad duration prior, not the movement adjustment. Otherwise, the app shows a broad historical duration prior or marks the resume estimate unavailable. MRMS lightning timestamps resolve to the one-minute observation window, not an exact strike time. Compact forecast snapshots are retained across seasons and older daily files are gzip compressed for prospective calibration; historical MRMS replay is unavailable from the rolling public feed.
